...
public class GameLogic {
    private List<LudosPlayer> players = new ArrayList<>();
    ...
    public void setLimitOfPlayers(int numberOfPlayers) {this.limitOfPlayers = Math.ceil(numberOfPlayers/2);}
    ...
    public void onPlayerInBassin(LudosPlayer player) {
        ...
        Core.get().getMapManager().getCurrentMap().getWorld().setBlockData(new Location(Core.get().getMapManager().getCurrentMap().getWorld(),
                player.entity().getLocation().getBlockX(),
                        bassinPosition.getY() - 1,
                        player.entity().getLocation().getBlockZ()),
                ...
                for (int z = this.boundary2Position.getBlockZ(); z <= this.boundary1Position.getBlockZ(); z++) {
                    for (int x = this.boundary2Position.getBlockX(); x <= this.boundary1Position.getBlockX(); x++) {
                        Core.get().getMapManager().getCurrentMap().getWorld()
                                .getBlockAt(x, y + decalagePlateforme, z).setType(Core.get().getMapManager().getCurrentMap().getWorld().getBlockAt(x, y, z).getType());
    ...
    public void nextPlayer(LudosPlayer player) {
        ...
        if (players.size()==1) {
            TitleUtils.broadcastTitle("&9" + "Victoire de " + players.get(0).entity().getName(),
                    "", 0, 2, 0.5f);
